...
const FloatingSocials = () => {
  ...
  // Hide social buttons when reaching footer (leave only the chat button)
  useEffect(() => {
    const checkFooterIntersection = () => {
      ...
      // Retract social buttons when footer comes into view
      const isNearFooter = footerRect.top <= windowHeight - 40;
      setIsVisible(!isNearFooter);
    };

    window.addEventListener('scroll', checkFooterIntersection, { passive: true });
    window.addEventListener('resize', checkFooterIntersection);
    checkFooterIntersection();
    return () => {
      window.removeEventListener('scroll', checkFooterIntersection);
      window.removeEventListener('resize', checkFooterIntersection);
    };
  }, []);
  ...
  return (
    <>
      {/* Highest z-index z-[100] across entire page so it floats above side poster banners and footer */}
      <div className="fixed bottom-6 right-5 sm:right-6 z-[100] flex flex-col items-end gap-3 pointer-events-none">
        {/* Chatbot peek preview (desktop only) */}
        {!isMobile && isChatPeeking && !isChatOpen && (
          <div
            className="chatbot-peek-container pointer-events-auto mb-1"
            onMouseEnter={handleChatHover}
            onMouseLeave={handleChatLeave}
          >
            <div className="chatbot-peek-bubble shadow-2xl border border-glass">
              <div className="flex items-center gap-2 mb-2">
                <img src="/avt.jpeg" alt="MVD" className="w-7 h-7 rounded-full object-cover" />
                <span className="font-bold text-accent text-xs">MVD AI Assistant</span>
              ...
              <p className="text-xs text-text-secondary leading-relaxed">
                Chào bạn! 👋 MVD AI có thể hỗ trợ gì cho bạn hôm nay?
              </p>
        ...
        {!isMobile && isChatOpen && (
          <div className="fixed right-[95px] sm:right-[100px] bottom-16 pointer-events-auto z-[110]">
      ...
      {isMobile && isChatOpen && (
        <div className="chatbot-mobile-overlay z-[200]">
