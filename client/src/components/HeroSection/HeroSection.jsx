...
const HeroSection = () => {
  ...
  return (
    <section className="relative h-screen min-h-[600px] px-4 w-full flex items-center justify-center overflow-hidden">
      {loading ? (
        <LoadingSpinner />
      ) : (
        <>
          ...
          <div className="container-custom relative z-20 text-center max-w-[800px] mx-auto bg-black/40 backdrop-blur-md p-8 md:p-12 rounded-3xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
            <h1 className="animate-fade-in text-[clamp(36px,5vw,64px)] mb-6 opacity-0 text-[#ffffff] drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)]">{data.title}</h1>
            <p className="animate-fade-in text-[clamp(16px,2vw,20px)] text-[#ffffff]/90 mb-10 max-w-[600px] mx-auto opacity-0 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]" style={{ animationDelay: '0.2s' }}>{data.subtitle}</p>
            <a
              href={data.ctaLink}
